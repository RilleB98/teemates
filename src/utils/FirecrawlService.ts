...
export class FirecrawlService {
  ...
  static async crawlGolfCourses(url: string): Promise<{ success: boolean; error?: string; data?: any }> {
    ...
    try {
      console.log('Making crawl request to Firecrawl API for golf courses');
  ...
  static async scrapeGolfCoursePage(url: string): Promise<{ success: boolean; error?: string; data?: any }> {
    const apiKey = this.getApiKey();
    if (!apiKey) {
      return { success: false, error: 'API key not found' };
    }
    ...
    try {
      console.log('Scraping single golf course page');
      if (!this.firecrawlApp) {
        this.firecrawlApp = new FirecrawlApp({ apiKey });
      }

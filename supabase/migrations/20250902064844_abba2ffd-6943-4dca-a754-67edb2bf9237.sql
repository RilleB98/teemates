...
FOR SELECT USING (
  ...
  (EXISTS ( 
    ...
    WHERE (friends.status = 'accepted'::text) AND 
          (((friends.user_id = (select auth.uid())) AND (friends.friend_id = profiles.user_id)) OR 
           ((friends.user_id = profiles.user_id) AND (friends.friend_id = (select auth.uid()))))
  ))
);
...
FOR SELECT USING (
  ...
  ((user_id = (select auth.uid())) OR 
   (EXISTS ( 
     SELECT 1 FROM friends
     WHERE (friends.status = 'accepted'::text) AND 
           (((friends.user_id = (select auth.uid())) AND (friends.friend_id = round_suggestions.user_id)) OR 
            ((friends.user_id = round_suggestions.user_id) AND (friends.friend_id = (select auth.uid()))))
   )))
);
...
FOR INSERT WITH CHECK (
  ...
  (EXISTS ( 
    ...
    JOIN friends f ON (((f.user_id = (select auth.uid())) AND (f.friend_id = rs.user_id)) OR 
                       ((f.user_id = rs.user_id) AND (f.friend_id = (select auth.uid()))))
...
FOR SELECT USING (
  EXISTS ( 
    SELECT 1 FROM round_suggestions rs
    WHERE (rs.id = round_suggestion_participants.round_suggestion_id) AND 
          ((rs.user_id = (select auth.uid())) OR 
           (EXISTS ( 
             SELECT 1 FROM friends
             WHERE (friends.status = 'accepted'::text) AND 
                   (((friends.user_id = (select auth.uid())) AND (friends.friend_id = rs.user_id)) OR 
                    ((friends.user_id = rs.user_id) AND (friends.friend_id = (select auth.uid()))))
           )))
  )
);
...
FOR SELECT USING (
  ...
  (EXISTS ( 
    ...
    WHERE (group_chat_members.group_chat_id = group_chats.id) AND 
          (group_chat_members.user_id = (select auth.uid()))
  ))
);
...
FOR INSERT WITH CHECK (
  ...
  (EXISTS ( 
    ...
    WHERE (group_chats.id = group_chat_members.group_chat_id) AND 
          (group_chats.created_by = (select auth.uid()))
  ))
);

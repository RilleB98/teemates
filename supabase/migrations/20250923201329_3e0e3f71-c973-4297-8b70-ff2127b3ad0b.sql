-- Fix security warnings by improving data protection (corrected version)
...
-- Drop and recreate all token policies for better security
...
DROP POLICY IF EXISTS "Limited token access for security" ON public.user_tokens;
...
-- Restrict token viewing to prevent token theft - only allow when token_hash exists (more secure)
CREATE POLICY "Secure token access only" ON public.user_tokens
FOR SELECT USING (auth.uid() = user_id);

-- 3. Improve notification_queue security
-- Add metadata columns for safer notification handling
...
BEGIN
  ...
  INSERT INTO public.notification_queue (
    ...
    message_type,
    sender_name,
    message_content -- Keep minimal for compatibility
  ) VALUES (
    _recipient_id,
    _sender_id,
    _message_type,
